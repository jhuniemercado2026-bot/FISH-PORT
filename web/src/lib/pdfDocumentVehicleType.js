...
export const buildVehicleTypePdf = async ({
  year,
  yearlyDate,
  preparedBy = "Admin",
  reportData = {},
  ...
  const vehicleTypes = Array.isArray(reportData.vehicleTypes) ? reportData.vehicleTypes : [];
  const generatedOn = new Date().toLocaleDateString("en-PH", {
    month: "long",
    day: "numeric",
    year: "numeric",
  });
  const coverageYear = String(yearlyDate || year || "-");
  ...
  composer.drawCenteredText("Vehicle Type Report", textCenterX, rightLowerSectionCenterY - 3, {
    fontSize: 12,
    bold: true,
  });
  ...
  const secondRowItems = [
    ["Municipality", "Opol"],
    ["Region", "X"],
    ["Total Vehicle Types", String(vehicleTypes.length)],
  ];
  ...
  const columns = [
    { key: "type_name", label: "Vehicle Type", width: balancedColumnWidth },
    { key: "daily_tickets_using", label: "Daily Tickets", width: balancedColumnWidth },
    { key: "annual_tickets_using", label: "Annual Tickets", width: balancedColumnWidth },
  ];

  drawTableHeader(composer, columns);

  const rows = vehicleTypes.length > 0
    ? vehicleTypes.map((type) => ({
        type_name: type?.type_name || type?.typeName || "-",
        daily_tickets_using: Number(type?.daily_tickets_using ?? 0),
        annual_tickets_using: Number(type?.annual_tickets_using ?? 0),
      }))
    : [
        {
          type_name: "-",
          daily_tickets_using: 0,
          annual_tickets_using: 0,
        },
      ];
  ...
  rows.forEach((row) => {
    ...
    columns.forEach((column, index) => {
      ...
      const cellValue = column.key === "daily_tickets_using" || column.key === "annual_tickets_using"
        ? String(row[column.key] ?? 0)
        : String(row[column.key] ?? "-");

      composer.drawText(cellValue, x + 6, composer.cursorY - 15, {
        fontSize: 8,
        color: COLORS.black,
      });
      x += column.width;
